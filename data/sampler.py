    ...
    def compute_indices(self):
        ...
        for axis in range(n_axis):
            if axis_indices[axis][-1] != self.image_size[axis] - self.patch_size[axis]:
                axis_indices[axis] = np.append(
                    axis_indices[axis], [self.image_size[axis] - self.patch_size[axis]], axis=0
                )
        axis_indices = np.meshgrid(*axis_indices, indexing="ij")
        indices = np.column_stack([axis_indices[axis].ravel() for axis in range(n_axis)])
        return indices
...
class ResizeSampler(Dataset):
    def __init__(self, sampler, target_size, image_size=None, patch_size=None, patch_overlap=None, mode="edge"):
        ...
        self.resized_sampler = GridSampler(image_size=image_size, patch_size=patch_size, patch_overlap=patch_overlap)
        self.sanity_check()

    def sanity_check(self):
        if len(self.sampler) != len(self.resized_sampler):
            raise RuntimeError(
                "Lengths of sampler ({}) and resized_sampler ({}) do not match.".format(
                    self.sampler.length[-1], self.resized_sampler.length[-1]
                )
            )
    ...
    def __getitem__(self, idx):
        ...
        if len(output) == 2 and not isinstance(output[0], dict):
            ...
            for key in patch_dict.keys():
                patch_dict[key] = ski_transform.resize(
                    patch_dict[key], output_shape=self.target_size, order=1, mode=self.mode
                )
            # patch_indices = self.resize_indices(patch_indices)
...
class ChunkedResizeSampler(Dataset):
    def __init__(
        self, sampler, target_size, image_size=None, patch_size=None, patch_overlap=None, chunk_size=None, mode="edge"
    ):
        self.sampler = sampler
        self.target_size = target_size
        self.mode = mode
        self.resized_sampler = ChunkedGridSampler(
